package com.retech.nyscms.entity;

import java.io.Serializable;
import java.util.Date;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;

/**
 * Table: front_virtual_coin
 * Author: qinc
 * Date: 2019-03-04 11:42:34
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
@Builder(toBuilder = true)
public class FrontVirtualCoin implements Serializable {
    /**
     * 主键
     * Column:    seqid
     * Nullable:  false
     */
    private Integer seqid;

    /**
     * 充值名称
     * Column:    virtualcoinType
     * Nullable:  true
     */
    private String virtualcointype;

    /**
     * 虚拟币金额
     * Column:    virtualcoinSum
     * Nullable:  true
     */
    private Float virtualcoinsum;

    /**
     * 赠送虚拟币金额
     * Column:    presenterSum
     * Nullable:  true
     */
    private Float presentersum;

    /**
     * 国内人名币
     * Column:    domesticPrice
     * Nullable:  true
     */
    private Float domesticprice;

    /**
     * 国外人名币
     * Column:    foreignPrice
     * Nullable:  true
     */
    private Float foreignprice;

    /**
     * 国内美元
     * Column:    domesticDollar
     * Nullable:  true
     */
    private Float domesticdollar;

    /**
     * 国外美元
     * Column:    foreignDollar
     * Nullable:  true
     */
    private Float foreigndollar;

    /**
     * 是否是苹果支付
     * Column:    isApplePay
     * Nullable:  true
     */
    private Integer isapplepay;

    /**
     * 创建时间
     * Column:    createdtime
     * Nullable:  false
     */
    private Date createdtime;

    /**
     * Column:    creatorid
     * Nullable:  true
     */
    private Integer creatorid;

    /**
     * 状态
     * Column:    status
     * Nullable:  true
     */
    private Integer status;

    private static final long serialVersionUID = 1L;
}